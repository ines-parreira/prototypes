...
} from '../useRevenuePerInteractionPerSalesAgentChannel'
...
jest.mock('domains/reporting/models/scopes/aiSalesAgentActivity', () => ({
    aiAgentSalesRevenuePerInteractionPerChannelQueryV2Factory: jest.fn(),
}))
...
).aiAgentSalesRevenuePerInteractionPerChannelQueryV2Factory as jest.Mock
...
const defaultAllValues = [
    ...
    { dimension: 'chat', value: 3.2, decile: null },
]

describe('useRevenuePerInteractionPerSalesAgentChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
...
describe('fetchRevenuePerInteractionPerSalesAgentChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchRevenuePerInteractionPerSalesAgentChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
    ...
    it('returns the result from fetchStatsMetricPerDimension', async () => {
        const result = await fetchRevenuePerInteractionPerSalesAgentChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
