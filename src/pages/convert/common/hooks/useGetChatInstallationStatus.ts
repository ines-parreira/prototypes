...
export const getChatInstallationStatus = (
    ...
    const defaultInstallation = { installed: true, method: null }

    if (integration === undefined || !integration.meta?.shop_integration_id) {
        return defaultInstallation
    }

    const installationMethod = integration.meta?.one_click_installation_method
    ...
    if (!uninstallationDatetime.isValid()) {
        return { installed: true, method: installationMethod }
    }
...
const useGetChatInstallationStatus = (
    ...
    return useMemo(() => getChatInstallationStatus(integration), [integration])
}
