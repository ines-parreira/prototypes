...
export const getOtherAgents = createSelector(
    ...
    (agents: List<*>, currentUser: currentUserType) =>
        agents.filter(
            (agent) =>
                String(agent.get('id', '')) !==
...
export const getAgent = (id: number) =>
    createSelector([getAgents], (agents: agentsType) => {
        if (!id) {
            return fromJS({})
        }
        ...
        return (
            agents.find(
                (agent) => agent.get('id', '').toString() === id.toString()
            ) || fromJS({})
        )
    })

export const makeGetAgent = (state: stateType) => (id: number): Map<*, *> =>
    getAgent(id)(state)
...
export const getAgentsIdsOnTicket = (ticketId: ticketIdType) =>
    createSelector([getAgentsIdsLocation], (locations: Array<Map<*, *>>) => {
        if (!ticketId) {
            return fromJS([])
        }
...
export const getAgentsOnTicket = (ticketId: ticketIdType) =>
    createSelector(
        [getAgentsIdsOnTicket(ticketId), makeGetAgent],
        //$FlowFixMe
        (
            agentsIds: List<Map<*, *>>,
            getUserObject: (Map<*, *>) => Map<*, *>
        ) => {
            if (!ticketId) {
                return fromJS([])
            }
...
export const makeGetOtherAgentsOnTicket = (
    state: stateType
): thunkActionType => (ticketId: ticketIdType) =>
...
export const getAgentsIdsTypingStatusOnTicket = (ticketId: ticketIdType) =>
    createSelector(
...
export const getAgentsTypingOnTicket = (ticketId: ticketIdType) =>
    createSelector(
...
export const getOtherAgentsTypingOnTicket = (ticketId: ticketIdType) =>
    createSelector(
        [
            currentUserSelectors.getCurrentUser,
...
export const isAgentTypingOnTicket = (ticketId: ticketIdType) =>
    createSelector(
        [
            currentUserSelectors.getCurrentUser,
            getAgentsTypingOnTicket(ticketId),
