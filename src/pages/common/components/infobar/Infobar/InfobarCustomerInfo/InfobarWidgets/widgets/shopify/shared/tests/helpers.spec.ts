...
describe('shortenUrl()', () => {
    it('should return shortened url when url is longer than 20 character with protocol', () => {
    ...
    it('should return shortened url when url is longer than 20 character without protocol', () => {
        const url = 'acme.gorgias.docker/app/customer/101'
        const result = shortenUrl(url)
        expect(result).toEqual('acme.gorgias.docker/...')
    })

    it('should return shortened url when url is shorter than 20 character with protocol', () => {
        const url = 'https://acme.gorgias.docker'
        const result = shortenUrl(url)
        expect(result).toEqual('acme.gorgias.docker')
    })

    it('should return shortened url when url is shorter than 20 character without protocol', () => {
