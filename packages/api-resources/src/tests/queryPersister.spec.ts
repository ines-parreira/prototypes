...
describe('shouldDehydrateQuery', () => {
    it('returns true for successful queries', () => {
        expect(shouldDehydrateQuery(createMockQuery())).toBe(true)
    })

    it('returns false for queries that are not successful', () => {
        expect(
            shouldDehydrateQuery(createMockQuery({ status: 'loading' })),
        ).toBe(false)
        expect(shouldDehydrateQuery(createMockQuery({ status: 'error' }))).toBe(
            false,
        )
    })

    it('returns false for queries with cacheTime 0', () => {
        expect(shouldDehydrateQuery(createMockQuery({ cacheTime: 0 }))).toBe(
            false,
        )
    })

    it('returns true for queries with a non-zero cacheTime', () => {
        expect(
            shouldDehydrateQuery(createMockQuery({ cacheTime: 60000 })),
        ).toBe(true)
    })
})
