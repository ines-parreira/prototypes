...
export default class RestrictedRevenue extends Component<Props> {
    render() {
        ...
        const imagesURL = [`${assetsURL}/static/private/img/presentationals/revenue-presentation.png`,]
        let alertMsg

        if (!hasRequiredIntegration) {
            alertMsg = <>
                Your e-commerce store needs to be connected to Gorgias to use this feature.
                <Link to="/app/settings/integrations/shopify/new"> Add one here</Link>
            </>
        } else if (!hasFeature) {
            alertMsg = <>
