...
describe('validationUtils', () => {
    ...
    describe('validateCustomerLookupStep', () => {
        it('should return no errors for valid customer lookup step', () => {
            const step = mockCustomerFieldsConditionalStep({
                custom_field_id: 1,
                branch_options: [],
            })

            const errors = validateCustomerLookupStep(step)

            expect(errors).toEqual([])
        })

        it('should return error when custom_field_id is missing', () => {
            const step = mockCustomerFieldsConditionalStep({
            ...
            expect(errors).toEqual(['Customer field is required'])
        })

        it('should return error when branch_options has no field_value', () => {
            const step = mockCustomerFieldsConditionalStep({
                custom_field_id: 1,
                branch_options: [
                    {
            ...
            expect(errors).toEqual(['Field value is required'])
        })
    })
})
