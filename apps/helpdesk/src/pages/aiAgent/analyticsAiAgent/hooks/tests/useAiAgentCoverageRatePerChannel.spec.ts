...
} from '../useAiAgentCoverageRatePerChannel'
...
jest.mock('domains/reporting/models/scopes/aiAgentCoverageRate', () => ({
    aiAgentCoverageRatePerChannelQueryFactoryV2: jest.fn(),
}))
...
).aiAgentCoverageRatePerChannelQueryFactoryV2 as jest.Mock
...
const MOCK_QUERY = { metricName: 'ai-agent-coverage-rate-per-channel' }

const defaultAllValues = [
    { dimension: 'email', value: 0.85, decile: null },
    { dimension: 'chat', value: 0.92, decile: null },
]

describe('useAiAgentCoverageRatePerChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
    ...
    it('calls the query factory with filters and timezone', () => {
        renderHook(() =>
            useAiAgentCoverageRatePerChannel(MOCK_STATS_FILTERS, MOCK_TIMEZONE),
        )
...
describe('fetchAiAgentCoverageRatePerChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchAiAgentCoverageRatePerChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
    ...
    it('returns the result from fetchStatsMetricPerDimension', async () => {
        const result = await fetchAiAgentCoverageRatePerChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
