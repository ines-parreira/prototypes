...
export default function useDraftMessages(isTicketNew = false) {
    ...
    type ReduxState = typeof reduxState
    const [ticketDraft, setTicketDraft] = useState<ReduxState | null>(null)
    ...
    useEffectOnce(() => {
        async function fetchTicketDraft() {
            const draft = (await localForage.getItem('new')) as ReduxState
    ...
    const shouldSaveDraft = useMemo(
        () =>
            isTicketNew &&
            ((ticketDraft !== null && !isNewTicketEmpty) ||
                (ticketDraft === null && !isNewTicketEmpty)),
        [isTicketNew, isNewTicketEmpty, ticketDraft]
    )
    ...
    useEffect(() => {
        if (previousIsNewTicketEmpty === false && isNewTicketEmpty) {
            persist()
        }
    }, [isNewTicketEmpty, persist, previousIsNewTicketEmpty])

    useEffect(() => {
        if (isTicketNew && ticketDraft !== null && isNewTicketEmpty) {
            hydrate()
        }
