...
describe('<GorgiasChatIntegrationAppearance/>', () => {
    ...
    describe('render()', () => {
        ...
        describe('conditional chat preview rendering based on AI agent access', () => {
            ...
            it('should render chat preview when revamp is disabled', () => {
                mockUseFlag.mockImplementation((key, defaultValue) => {
            ...
            it('should render chat preview when revamp is enabled but user does not have AI agent access', () => {
                mockUseFlag.mockImplementation((key, defaultValue) => {
                    if (key === FeatureFlagKey.ChatSettingsRevamp) {
                        return true
                    }
            ...
            it('should not render chat preview when revamp is enabled and user has AI agent access', () => {
                mockUseFlag.mockImplementation((key, defaultValue) => {
                    if (key === FeatureFlagKey.ChatSettingsRevamp) {
                        return true
                    }
                ...
                expect(
                    screen.queryByText('During Business Hours'),
                ).not.toBeInTheDocument()
                expect(
                    screen.queryByText('Outside Business Hours'),
                ).not.toBeInTheDocument()
                expect(container).toMatchSnapshot()
            })

            it('should correctly find and pass the matching store integration to useRevampShouldShowChatPreview', () => {
                const store1ShopName = 'Test Store 1'
                const store2ShopName = 'Test Store 2'
                const store3ShopName = 'Test Store 3'
                ...
                mockUseAiAgentAccess.mockImplementation((shopName) => {
                    if (shopName === store2ShopName) {
                        return { hasAccess: true, isLoading: false }
                    }
                    return { hasAccess: false, isLoading: false }
                })

                const integrationWithStore2 = fromJS({
                    id: 1,
                    name: 'Test Chat',
                    type: GORGIAS_CHAT_INTEGRATION_TYPE,
                    decoration: {
                ...
                const multipleStoreIntegrations = fromJS([
                    {
                        id: 1,
                        name: store1ShopName,
                        type: SHOPIFY_INTEGRATION_TYPE,
                        meta: {
                            shop_name: store1ShopName,
                        },
                    },
                    {
                        id: 2,
                        ...
                        <QueryClientProvider client={mockClient}>
                            <Provider store={mockStore(defaultState)}>
                                <GorgiasChatIntegrationAppearanceComponent
                                    {...minProps}
                                    storeIntegrations={
                                        multipleStoreIntegrations
                                    }
                                    integration={integrationWithStore2}
                                    loading={fromJS({
                                        updateIntegration: false,
                                    })}
                                    currentUser={mockCurrentUser}
                ...
                expect(mockUseAiAgentAccess).toHaveBeenCalledWith(
                    store2ShopName,
                )
                expect(
                    screen.queryByText('During Business Hours'),
            ...
            it('should handle undefined store integration when no matching shop_integration_id exists', () => {
                mockUseFlag.mockImplementation((key, defaultValue) => {
                    if (key === FeatureFlagKey.ChatSettingsRevamp) {
                        return true
                    }
                ...
                const integrationWithNonExistentStore = fromJS({
                    id: 1,
                    name: 'Test Chat',
                    type: GORGIAS_CHAT_INTEGRATION_TYPE,
                    decoration: {
                        ...
                        <QueryClientProvider client={mockClient}>
                            <Provider store={mockStore(defaultState)}>
                                <GorgiasChatIntegrationAppearanceComponent
                                    {...minProps}
                                    integration={
                                        integrationWithNonExistentStore
                                    }
                                    loading={fromJS({
                                        updateIntegration: false,
                                    })}
            ...
            it('should handle undefined store integration when shop_integration_id is null', () => {
                mockUseFlag.mockImplementation((key, defaultValue) => {
                    if (key === FeatureFlagKey.ChatSettingsRevamp) {
                        return true
                    }
                ...
                const integrationWithoutStoreId = fromJS({
                    id: 1,
                    name: 'Test Chat',
                    type: GORGIAS_CHAT_INTEGRATION_TYPE,
                    decoration: {
                    ...
                    <Router history={history}>
                        <QueryClientProvider client={mockClient}>
                            <Provider store={mockStore(defaultState)}>
                                <GorgiasChatIntegrationAppearanceComponent
                                    {...minProps}
                                    integration={integrationWithoutStoreId}
                                    loading={fromJS({
                                        updateIntegration: false,
                                    })}
                                    currentUser={mockCurrentUser}
