...
jest.mock('domains/reporting/models/scopes/overallTimeSavedByAgent', () => ({
    overallTimeSavedByAgentPerFeatureQueryFactoryV2: jest.fn(),
}))
...
).overallTimeSavedByAgentPerFeatureQueryFactoryV2 as jest.Mock
...
const MOCK_QUERY = { metricName: 'overall-time-saved-by-agent-per-feature' }

const defaultAllValues = [
    { dimension: 'ai-agent', value: 7200, decile: null },
    { dimension: 'flow', value: 3600, decile: null },
    { dimension: 'article-recommendation', value: 1800, decile: null },
    { dimension: 'order-management', value: 900, decile: null },
]

describe('useTimeSavedPerFeature', () => {
    beforeEach(() => {
        jest.clearAllMocks()
    ...
    it('calls the query factory with filters and timezone', () => {
        renderHook(() =>
            useTimeSavedPerFeature(MOCK_STATS_FILTERS, MOCK_TIMEZONE),
        )
...
describe('fetchTimeSavedPerFeature', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchTimeSavedPerFeature(MOCK_STATS_FILTERS, MOCK_TIMEZONE)
    ...
    it('returns the result from fetchStatsMetricPerDimension', async () => {
        const result = await fetchTimeSavedPerFeature(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
