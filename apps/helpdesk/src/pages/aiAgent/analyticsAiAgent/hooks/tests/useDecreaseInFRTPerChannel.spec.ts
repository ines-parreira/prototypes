...
} from '../useDecreaseInFRTPerChannel'
...
jest.mock(
    ...
    () => ({
        aiAgentSupportAgentDecreaseInFRTPerChannelQueryFactoryV2: jest.fn(),
    }),
)
...
const mockQueryFactory = jest.requireMock(
    'domains/reporting/models/scopes/aiAgentDecreaseInFirstResponseTime',
).aiAgentSupportAgentDecreaseInFRTPerChannelQueryFactoryV2 as jest.Mock
...
const defaultAllValues = [
    { dimension: 'email', value: 300, decile: null },
    { dimension: 'chat', value: 150, decile: null },
]

describe('useDecreaseInFRTPerChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
    ...
    it('calls the query factory with filters and timezone', () => {
        renderHook(() =>
            useDecreaseInFRTPerChannel(MOCK_STATS_FILTERS, MOCK_TIMEZONE),
        )
...
describe('fetchDecreaseInFRTPerChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchDecreaseInFRTPerChannel(MOCK_STATS_FILTERS, MOCK_TIMEZONE)
    ...
    it('returns the result from fetchStatsMetricPerDimension', async () => {
        const result = await fetchDecreaseInFRTPerChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
