...
export const getTicketTranslationHelper = ({
    language,
    primary,
    isTranslated,
}: {
    ...
    return `Translate ticket to ${IntlDisplayNames.of(primary)}`
}
...
export const useTicketTranslationHelper = (language: Language | null) => {
    ...
    return useMemo(() => {
        return getTicketTranslationHelper({
            language,
            primary: primary as Language | null,
            isTranslated:
