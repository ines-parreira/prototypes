...
export const getViews = createImmutableSelector([getViewsState], (state) =>
    state.get('items', fromJS([]))
)
...
export const getPristineActiveView = createImmutableSelector(
    ...
    (views, activeView) =>
        views.find((v) => v.get('id') === activeView.get('id')) || fromJS({})
)
...
export const getLastViewId = createSelector([getViewsState], (state) =>
    state.getIn(['_internal', 'lastViewId'])
)

export const getLoading = createSelector([getViewsState], (state) =>
    state.getIn(['_internal', 'loading'], fromJS({}))
)
...
export const getViewsByType = (type: string) =>
...
export const getViewIdToDisplay = (type: string, urlViewId: ?string) =>
    createSelector([getViewsByType(type)], (views: List<Map<*, *>>) => {
        if (urlViewId) {
            // Prevent suggesting a view with the wrong type
            const matchingView = views.find(
                (view) => view.get('id') === parseInt(urlViewId)
            )
            if (matchingView) {
...
export const makeGetViewIdToDisplay = (state: stateType) => (
    type: string,
    urlViewId: ?string
 ...
 */
export const getView = (id: string, configName: ?string = '') =>
    createImmutableSelector([getViews], (views) => {
        ...
        return views.find(
            (view) => view.get('id') === parseInt(id),
            null,
            fromJS({})
        )
    })

export const makeGetView = (state: stateType) => (
    id: string,
    configName: ?string
...
export const makeGetViewCount = (state: stateType) => (viewId: string) =>
