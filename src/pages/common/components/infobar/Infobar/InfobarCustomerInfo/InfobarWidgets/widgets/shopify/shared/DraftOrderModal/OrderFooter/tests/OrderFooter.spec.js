...
describe('<DuplicateOrderFooterComponent/>', () => {
    ...
    describe('_onNoteChange()', () => {
        it.each([
            [ShopifyAction.CREATE_ORDER, EVENTS.SHOPIFY_CREATE_ORDER_NOTES_CHANGED],
            [ShopifyAction.DUPLICATE_ORDER, EVENTS.SHOPIFY_DUPLICATE_ORDER_NOTES_CHANGED],
        ])('should call onPayloadChange() with updated payload', (actionName, event) => {
    ...
    describe('_onTagsChange()', () => {
        it.each([
            [ShopifyAction.CREATE_ORDER, EVENTS.SHOPIFY_CREATE_ORDER_TAGS_CHANGED],
            [ShopifyAction.DUPLICATE_ORDER, EVENTS.SHOPIFY_DUPLICATE_ORDER_TAGS_CHANGED],
        ])('should call onPayloadChange() with updated payload', (actionName, event) => {
            const payload = fromJS(shopifyDraftOrderPayloadFixture())
