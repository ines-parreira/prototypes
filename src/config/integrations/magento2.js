...
export const MACRO_VARIABLES = {
    ...
    children: [{
        ...
        replace: (context: Map<*, *>, integrationId: number) => {
            const lastOrder = context.getIn(['ticket', 'customer', 'integrations', integrationId, 'orders', 0])
            const shipments = context.getIn(['ticket', 'customer', 'integrations', integrationId, 'shipments'])
            if (!lastOrder || !shipments) {
                return ''
            }

            const lastOrderLastShipment = shipments
                .filter((shipment) => shipment.get('order_id') === lastOrder.get('entity_id'))
                .first()
        ...
        replace: (context: Map<*, *>, integrationId: number) => {
            ...
            return formatDatetime(lastTrack.get('created_at'), null, LDMLToMomentFormat('MMMM d YYYY'))
        }
    }, {
