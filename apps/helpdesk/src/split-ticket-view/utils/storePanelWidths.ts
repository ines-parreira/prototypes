...
const storePanelWidths = debounce((layoutKey: LayoutKeys, widths: number[]) => {
    if (widths.length >= 2) {
        tryLocalStorage(() => {
            window.localStorage.setItem('navbar-width', widths[0].toString())
            ...
            window.localStorage.setItem(layoutKey, `v2;${widths.toString()}`)
        })
    }

    if (layoutKey === LayoutKeys.TICKET && widths.length === 4) {
        tryLocalStorage(() => {
            window.localStorage.setItem('infobar-width', widths[3].toString())
        })
    }

    if (layoutKey === LayoutKeys.FULL_TICKET && widths.length === 3) {
        tryLocalStorage(() => {
            window.localStorage.setItem('infobar-width', widths[2].toString())
        })
    }
}, PANELS_STORAGE_DEBOUNCE_TIME)
