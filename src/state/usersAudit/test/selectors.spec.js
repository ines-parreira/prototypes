...
describe('users audit selectors', () => {
    ...
    it('getUserAuditEvents', () => {
        ...
        expect(selectors.getUserAuditEvents(state)).toEqualImmutable(state.usersAudit.get('events'))
    })

    it('getUserAuditPagination', () => {
        expect(selectors.getUserAuditPagination({})).toEqualImmutable(fromJS([]))
        expect(selectors.getUserAuditPagination(state)).toEqualImmutable(state.usersAudit.get('meta'))
    })

    it('getUserAuditUserIdOptions', () => {
        expect(selectors.getUserAuditUserIdOptions({})).toEqualImmutable(fromJS([]))
        expect(fromJS(selectors.getUserAuditUserIdOptions(state).toJS())).toEqualImmutable(fromJS([
            {label: '', value: 1},
            {label: '', value: 2},
        ]))
    })
    ...
    it('getUserAuditObjectTypeOptions', () => {
        expect(fromJS(selectors.getUserAuditObjectTypeOptions().toJS())).toEqualImmutable(fromJS([
    ...
    it('getUserAuditEventTypeOptions', () => {
        expect(fromJS(selectors.getUserAuditEventTypeOptions().toJS())).toEqualImmutable(fromJS([
