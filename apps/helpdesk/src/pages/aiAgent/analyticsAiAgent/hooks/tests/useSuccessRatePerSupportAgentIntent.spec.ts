...
} from '../useSuccessRatePerSupportAgentIntent'
...
jest.mock('domains/reporting/models/scopes/aiAgentSuccessRate', () => ({
    aiSupportAgentSuccessRatePerIntentQueryFactoryV2: jest.fn(),
}))
...
).aiSupportAgentSuccessRatePerIntentQueryFactoryV2 as jest.Mock
...
const MOCK_QUERY = { metricName: 'ai-agent-support-success-rate-per-intent' }

const defaultAllValues = [
    { dimension: 'Billing :: Refund', value: 0.82, decile: null },
    { dimension: 'Shipping :: Status', value: 0.71, decile: null },
]

describe('useSuccessRatePerSupportAgentIntent', () => {
    beforeEach(() => {
        jest.clearAllMocks()
...
describe('fetchSuccessRatePerSupportAgentIntent', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with stats filters and timezone', async () => {
        await fetchSuccessRatePerSupportAgentIntent(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
    ...
    it('returns the result from fetchStatsMetricPerDimension', async () => {
        const result = await fetchSuccessRatePerSupportAgentIntent(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
