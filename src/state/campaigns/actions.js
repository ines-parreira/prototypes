...
export function updateCampaign(campaign, integration) {
    return (dispatch) => {
        return axios.put(
            `/api/integrations/${integration.get('id')}/campaigns/${campaign.get('id')}`,
            campaign.toJS())
            .then((json = {}) => json.data)
            .then(() => {
                onUpdateSuccess(dispatch, integration.toJS(), null, true)
                return dispatch(fetchIntegration(integration.get('id'), integration.get('type')))
            }, (error) => {
                return dispatch({
                    type: types.UPDATE_INTEGRATION_ERROR,
                    error,
...
export function deleteCampaign(campaign, integration) {
    return (dispatch) => {
        return axios.delete(`/api/integrations/${integration.get('id')}/campaigns/${campaign.get('id')}`)
            .then((json = {}) => json.data)
            .then(() => {
                onUpdateSuccess(dispatch, integration.toJS(), null, true)
                return dispatch(fetchIntegration(integration.get('id'), integration.get('type')))
