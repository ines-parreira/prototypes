...
} from '../useTimeSavedByAgentPerChannel'
...
jest.mock('domains/reporting/models/scopes/aiAgentTimeSaved', () => ({
    overallTimeSavedByAgentPerChannelQueryFactoryV2: jest.fn(),
}))
...
).overallTimeSavedByAgentPerChannelQueryFactoryV2 as jest.Mock
...
const defaultAllValues = [
    { dimension: 'email', value: 7200, decile: null },
    { dimension: 'chat', value: 3600, decile: null },
]

describe('useTimeSavedByAgentPerChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
    ...
    it('calls the query factory with filters and timezone', () => {
        renderHook(() =>
            useTimeSavedByAgentPerChannel(MOCK_STATS_FILTERS, MOCK_TIMEZONE),
        )
...
describe('fetchTimeSavedByAgentPerChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchTimeSavedByAgentPerChannel(MOCK_STATS_FILTERS, MOCK_TIMEZONE)
    ...
    it('returns the result from fetchStatsMetricPerDimension', async () => {
        const result = await fetchTimeSavedByAgentPerChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
