...
export default class UsersNavbarView extends React.Component {
    _sections(views) {
        // Populate sections
        const ret = {
            favorites: {
    ...
    render() {
        ...
        const displayedViews = views
            .get('items', fromJS([]))
            .filter((view) => view.get('type') === 'user-list')
                                    ...
                                    <div className="menu">
                                        {
                                            section.views
                                                .map((v) => {
                                                    ...
                                                    return (
                                                        <Link key={key}
                                                              to={`/app/users/${view.id}/${view.slug}`}
                                                              className={classes}
                                                        >
