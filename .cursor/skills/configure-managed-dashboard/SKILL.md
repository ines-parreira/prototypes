...
description: Configure a "managed" analytics dashboard rendered by DashboardLayoutRenderer (e.g. Performance Overview). Has sub-modes for (A) adding the standardized value/breakdown/timeseries query factories on the scope and (B) wiring a new TrendCard component, its ReportConfig entry, the default layout slot, and the shared trend-card tests. Use when adding a metric card to a managed dashboard such as Performance Overview.
---
