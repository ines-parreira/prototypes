...
describe('customers helpers', () => {
    ...
    describe('mergeChannels()', () => {
        const channel1 = {address: 'address1@foo.bar', preferred: false, type: EMAIL_CHANNEL}
        const channel2 = {address: 'address2@foo.bar', preferred: false, type: EMAIL_CHANNEL}
        const sameChannel1 = {address: 'ADDRESS1@foo.bar', preferred: false, type: EMAIL_CHANNEL}

        it('should return all channels because emails are different', () => {
            expect(helpers.mergeChannels([channel1, channel2])).toEqual([channel1, channel2])
        })

        it('should return lowercase channel because emails are the same', () => {
            expect(helpers.mergeChannels([sameChannel1, channel1])).toEqual([channel1])
        })

        it('should return lowercase channel because emails are the same, lowercase channel received first', () => {
            expect(helpers.mergeChannels([channel1, sameChannel1])).toEqual([channel1])
        })
    })
})
