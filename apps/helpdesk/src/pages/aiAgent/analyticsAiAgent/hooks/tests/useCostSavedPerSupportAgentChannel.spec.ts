...
} from '../useCostSavedPerSupportAgentChannel'
...
describe('useCostSavedPerSupportAgentChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
...
describe('fetchCostSavedPerSupportAgentChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchCostSavedPerSupportAgentChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
    ...
    it('multiplies each entity value by the provided cost per interaction', async () => {
        const result = await fetchCostSavedPerSupportAgentChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
            COST_PER_INTERACTION,
        )
