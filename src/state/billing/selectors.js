...
export const currentPlanId = createSelector(
    ...
    (currentAccountState, billingState) => {
        return (
            currentAccountState.getIn(['current_subscription', 'plan']) ||
            billingState.get('futureSubscriptionPlan')
        )
    }
)

export const plans = createSelector([getBillingState], (state) => {
    return state
        .get('plans', fromJS({}))
...
export const getPlan = (planId: string) =>
    createSelector([plans], (p) => p.get(planId) || fromJS({}))

export const invoices = createSelector([getBillingState], (billing) =>
    billing
        .get('invoices', fromJS([]))
        .filter(
            (invoice) =>
...
export const planIntegrations = createSelector([currentPlan], (plan) =>
    plan.get('integrations', 0)
)
...
export const makeIsAllowedToChangePlan = (state: stateType) => (
    planId: string
