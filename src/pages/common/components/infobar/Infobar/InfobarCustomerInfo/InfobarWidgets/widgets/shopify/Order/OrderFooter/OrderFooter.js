...
export class OrderFooterComponent extends React.PureComponent<Props, State> {
    static contextTypes = {
    ...
    _trackNoteChanged = _debounce(() => {
        segmentTracker.logEvent(segmentTracker.EVENTS.SHOPIFY_DUPLICATE_ORDER_NOTES_CHANGED)
    }, 1000)
    ...
    _onTagsChange = (tags: Option[]) => {
        ...
        onPayloadChange(integrationId, newPayload)
        segmentTracker.logEvent(segmentTracker.EVENTS.SHOPIFY_DUPLICATE_ORDER_TAGS_CHANGED)
    }

    render() {
        const {editable, payload, currencyCode} = this.props
        const {note} = this.state
                    ...
                    >
                        ...
                        <div>
                            ...
                            <MultiSelectOptionsField
                                options={OrderFooterComponent.tagsToOptions(this._defaultTags)}
                                selectedOptions={OrderFooterComponent.tagsToOptions(tags)}
                                plural="tags"
                                singular="tag"
...
export default connect(mapStateToProps, mapDispatchToProps)(OrderFooterComponent)
