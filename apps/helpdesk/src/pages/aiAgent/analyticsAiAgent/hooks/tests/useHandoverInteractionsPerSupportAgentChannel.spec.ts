...
} from '../useHandoverInteractionsPerSupportAgentChannel'
...
jest.mock('domains/reporting/models/scopes/handoverInteractions', () => ({
    aiSupportHandoverInteractionsPerChannelQueryFactoryV2: jest.fn(),
}))
...
).aiSupportHandoverInteractionsPerChannelQueryFactoryV2 as jest.Mock
...
const MOCK_QUERY = {
    metricName: 'ai-agent-support-handover-interactions-per-channel',
}
...
describe('useHandoverInteractionsPerSupportAgentChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
...
describe('fetchHandoverInteractionsPerSupportAgentChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchHandoverInteractionsPerSupportAgentChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
    ...
    it('returns the result from fetchStatsMetricPerDimension', async () => {
        const result = await fetchHandoverInteractionsPerSupportAgentChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
