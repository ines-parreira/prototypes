...
describe('EmailForwardingButton', () => {
    ...
    it('Should display an error toast when verifying integration fails', async () => {
        const user = userEvent.setup()
        computeStatusSpy.mockReturnValue(EmailVerificationStatus.Unverified)
        ...
        await user.click(screen.getByText('Verify forwarding'))

        await waitFor(() => {
            const toast = screen.getByRole('status', { hidden: true })
            expect(toast).toHaveTextContent('Verification failed')
