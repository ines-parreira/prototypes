...
} from '../useTotalSalesPerSalesAgentChannel'
...
jest.mock(
    ...
    () => ({
        aiAgentSalesTotalSalesPerChannelQueryV2Factory: jest.fn(),
    }),
)
...
const mockQueryFactory = jest.requireMock(
    'domains/reporting/models/scopes/aiSalesAgentOrdersPerformance',
).aiAgentSalesTotalSalesPerChannelQueryV2Factory as jest.Mock
...
const defaultAllValues = [
    { dimension: 'email', value: 5000.5, decile: null },
    { dimension: 'chat', value: 3200.75, decile: null },
]

describe('useTotalSalesPerSalesAgentChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
...
describe('fetchTotalSalesPerSalesAgentChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchTotalSalesPerSalesAgentChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
    ...
    it('returns the result from fetchStatsMetricPerDimension', async () => {
        const result = await fetchTotalSalesPerSalesAgentChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
