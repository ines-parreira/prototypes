...
describe('ticket utils', () => {
    ...
    describe('replaceIntegrationVariables()', () => {
        it('should return an empty value and log a notification because there is no matching integration', () => {
            ...
            const res = replaceIntegrationVariables(SHOPIFY_INTEGRATION_TYPE, ticketState, variable, newArg, notifySpy)

            expect(res).toEqual('Hello {{ticket.customer.integration.shopify.customer.name}}, what is your {{}}?')
        ...
        it('should update the variable with the correct value using the replace function in the config of the variable',
            () => {
                ...
                const variable = '{{ticket.customer.integrations.shopify.customer.variableWithReplace}}'
                const newArg = 'Hello {{ticket.customer.integration.shopify.customer.name}}, ' +
                    'what is your {{ticket.customer.integrations.shopify.customer.variableWithReplace}}?'

                const res = replaceIntegrationVariables(SHOPIFY_INTEGRATION_TYPE, ticketState, variable, newArg,
                    notifySpy)
