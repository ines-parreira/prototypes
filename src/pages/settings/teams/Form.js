...
export default class Form extends Component<Props, State> {
    ...
    _fetchTeam = (id: number) => {
        ...
        return this.props.fetchTeam(id)
            .then((resp) => {
                this.setState({
                    isFetching: false,
                    team: resp
    ...
    _onSubmit = (e: SyntheticEvent<*>) => {
        ...
        const promise = this._isUpdate() ? this.props.updateTeam(team) : this.props.createTeam(team)
        return promise
            .then((resp) => {
                this.setState({isSubmitting: false})
                if (!this._isUpdate()) {
                    browserHistory.push(`app/settings/teams/${resp.get('id')}/members`)
                }
            })
    }
        ...
        return (
            <div className="full-width">
                <PageHeader title={(
                    <Breadcrumb>
                        ...
                        <BreadcrumbItem active>
                            {isUpdate ? `Edit ${team.get('name')}` : 'Create team'}
                        </BreadcrumbItem>
                    </Breadcrumb>
                )}/>

                {
                    isUpdate && (
                        <SecondaryNavbar>
                            <Link to={`/app/settings/teams/${team.get('id')}/members`}>Team members</Link>
                            <Link to={`/app/settings/teams/${team.get('id')}`}>Settings</Link>
                        </SecondaryNavbar>
                    )
                }
                <Container
                            ...
                            <Col lg={6}>
                                <div className={'d-flex'}>
                                    <InputField
                                        ...
                                        value={team.get('name') || ''}
                                        onChange={(value) => this.setState({team: team.set('name', value)})}
                                        required
                                    />
                                    <FormGroup className={'ml-2 align-self-end'}>
                                        <ButtonGroup>
                                            <Button
                                                type="button"
                                                color="secondary"
                                            ...
                                            >
                                                {
                                                    emoji ? (
                                                        <div className={'flex'}>
                                                            <div className={'mr-1 flex align-self-center'}>
                                                                <Emoji
                                                                    emoji={emoji.toJS()}
                                                                    size={16}
                                                                />
                                                ...
                                                emoji && (
                                                    <Button
                                                        ...
                                                        onClick={() => this.setState({
                                                            team: team.setIn(['decoration', 'emoji'], null)
                                                        })}
                                                    >
                                        ...
                                        <Popover
                                            placement={'right'}
                                            isOpen={this.state.isEmojiPickerOpen}
                                            target="add-emoji"
                                        ...
                                        >
                                            <PopoverBody className={'p-0'}>
                                                <Picker
                                                    ...
                                                    onClick={(emoji) => {
                                                        this.setState({
                                                            team: team.setIn(['decoration', 'emoji'], fromJS(emoji))
                                                        })
                                ...
                                <InputField
                                    ...
                                    onChange={(value) => this.setState({team: team.set('description', value)})}
                                />
                                <FormGroup>
                                    <Button
                                        type="submit"
                                    ...
                                    </Button>
                                    {
                                        isUpdate && (
                                            <ConfirmButton
                                                ...
                                                outline
                                            >
                                                <i className="material-icons">delete</i> Delete team
                                            </ConfirmButton>
                                        )
                                    }
                                </FormGroup>
