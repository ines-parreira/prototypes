...
} from '../useCostSavedPerSupportAgentIntent'
...
describe('useCostSavedPerSupportAgentIntent', () => {
    beforeEach(() => {
        jest.clearAllMocks()
    ...
    it('returns null for null values without multiplying', () => {
        ...
        const billingValue = result.current.data?.allValues?.find(
            (v) => v.dimension === 'Billing :: Refund',
        )
        expect(billingValue?.value).toBeNull()
    })
...
describe('fetchCostSavedPerSupportAgentIntent', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchCostSavedPerSupportAgentIntent(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
    ...
    it('multiplies each entity value by the provided cost per interaction', async () => {
        const result = await fetchCostSavedPerSupportAgentIntent(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
            COST_PER_INTERACTION,
        )
