...
} from '../useDecreaseInFRTPerSupportAgentIntent'
...
jest.mock(
    ...
    () => ({
        aiAgentSupportAgentDecreaseInFRTPerIntentQueryFactoryV2: jest.fn(),
    }),
)
...
const mockQueryFactory = jest.requireMock(
    'domains/reporting/models/scopes/aiAgentDecreaseInFirstResponseTime',
).aiAgentSupportAgentDecreaseInFRTPerIntentQueryFactoryV2 as jest.Mock
...
const MOCK_QUERY = {
    metricName: 'ai-agent-support-agent-decrease-in-frt-per-intent',
}
const defaultAllValues = [
    { dimension: 'Billing :: Refund', value: 180, decile: null },
    { dimension: 'Shipping :: Status', value: 75, decile: null },
]

describe('useDecreaseInFRTPerSupportAgentIntent', () => {
    beforeEach(() => {
        jest.clearAllMocks()
...
describe('fetchDecreaseInFRTPerSupportAgentIntent', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchDecreaseInFRTPerSupportAgentIntent(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
    ...
    it('returns the result from fetchStatsMetricPerDimension', async () => {
        const result = await fetchDecreaseInFRTPerSupportAgentIntent(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
