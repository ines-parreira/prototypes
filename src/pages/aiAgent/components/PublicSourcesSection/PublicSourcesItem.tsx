...
export const PublicSourcesItem = ({
    ...
    const handleSyncButtonClick = () => {
        // Only show confirmation modal if re-syncing an existing URL
        // For new URLs being added, sync directly without modal
        const isNewUrl = !source.url || source.url !== value

        if (isNewUrl) {
            // New URL - sync directly without modal
            handleSync()
        } else {
            // Existing URL - show confirmation modal
    ...
    useEffect(() => {
        if (!!syncUrlOnCommand && !isCurrentUrlPristine && !isSyncDisabled) {
            handleSync()
        }
