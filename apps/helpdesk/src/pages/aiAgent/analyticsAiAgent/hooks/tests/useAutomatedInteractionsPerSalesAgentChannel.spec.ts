...
} from '../useAutomatedInteractionsPerSalesAgentChannel'
...
jest.mock(
    ...
    () => ({
        aiSalesAgentAutomatedInteractionsPerChannelQueryFactoryV2: jest.fn(),
    }),
)
...
const mockQueryFactory = jest.requireMock(
    'domains/reporting/models/scopes/aiAgentAutomatedInteractions',
).aiSalesAgentAutomatedInteractionsPerChannelQueryFactoryV2 as jest.Mock
...
describe('useAutomatedInteractionsPerSalesAgentChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
...
describe('fetchAutomatedInteractionsPerSalesAgentChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchAutomatedInteractionsPerSalesAgentChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
    ...
    it('returns the result from fetchStatsMetricPerDimension', async () => {
        const result = await fetchAutomatedInteractionsPerSalesAgentChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
