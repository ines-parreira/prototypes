...
export function getParentNodeInPath(
    type: VisualBuilderNode['type'],
    graph: VisualBuilderGraph,
    nodeId: string
): VisualBuilderNode | null {
    let parentNode: VisualBuilderNode | null = null
...
export function replaceAliases(g: VisualBuilderGraph) {
    for (const node of g.nodes) {
        switch (node.type) {
            ...
            case 'order_selection': {
                if (hasOrderAlias(node.data.content.text)) {
                    const orderSelectionNode = getParentNodeInPath(
                        'order_selection',
                        g,
                        node.id
                    )

                    if (orderSelectionNode) {
                        node.data.content.text = replaceOrderAlias(
                            node.data.content.text,
                            orderSelectionNode.id
                        )
                        node.data.content.html = replaceOrderAlias(
                ...
                if (hasCustomerAlias(node.data.content.text)) {
                    const shopperAuthenticationNode = getParentNodeInPath(
                        'shopper_authentication',
                        g,
                        node.id
                    )

                    if (shopperAuthenticationNode) {
                        node.data.content.text = replaceCustomerAlias(
                            node.data.content.text,
                            shopperAuthenticationNode.id
                        )
                        node.data.content.html = replaceCustomerAlias(
                            node.data.content.html,
                            shopperAuthenticationNode.id
                        )
                    }
            ...
            case 'http_request': {
                ...
                if (orderSelectionNode) {
                    if (hasOrderAlias(node.data.url)) {
                        node.data.url = replaceOrderAlias(
                            node.data.url,
                            orderSelectionNode.id
                        )
                    }

                    if (node.data.json && hasOrderAlias(node.data.json)) {
                        node.data.json = replaceOrderAlias(
                            node.data.json,
                            orderSelectionNode.id
                        )
                    }
                    ...
                    node.data.headers.forEach((item) => {
                        if (hasOrderAlias(item.value)) {
                            item.value = replaceOrderAlias(
                                item.value,
                                orderSelectionNode.id
                            )
                        }
                ...
                if (shopperAuthenticationNode) {
                    if (hasCustomerAlias(node.data.url)) {
                        node.data.url = replaceCustomerAlias(
                            node.data.url,
                            shopperAuthenticationNode.id
                        )
                    }

                    if (node.data.json && hasCustomerAlias(node.data.json)) {
                        node.data.json = replaceCustomerAlias(
                            node.data.json,
                            shopperAuthenticationNode.id
                        )
                    }
                    ...
                    node.data.headers.forEach((item) => {
                        if (hasCustomerAlias(item.value)) {
                            item.value = replaceCustomerAlias(
                                item.value,
                                shopperAuthenticationNode.id
                            )
                        }
