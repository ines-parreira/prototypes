...
import { useSchedulerConfigV3 } from '../hooks/useSchedulerConfigV3'
import type { RefreshConfigV3 } from '../scheduler/refreshConfigV3'
import { getTtlSecondsForCount } from '../scheduler/refreshConfigV3'
...
import { viewsCountStoreV3 } from '../store/viewsCountStoreV3'

type ViewCountDebugPanelV3Props = {
    isOpen?: boolean
...
export function ViewCountDebugPanelV3({
    isOpen = false,
    onOpenChange,
}: ViewCountDebugPanelV3Props) {
    const config = useSchedulerConfigV3()
    const recent = useStore(viewsCountStoreV3, (s) => s.recent)
...
function LeaderCard() {
    const isLeader = useStore(viewsCountStoreV3, (s) => s.isLeader)
    return (
        <Tooltip
            trigger={
...
function NextTickCard() {
    const nextTickAt = useStore(viewsCountStoreV3, (s) => s.nextTickAt)
    const isLeader = useStore(viewsCountStoreV3, (s) => s.isLeader)
    const now = useNow()
...
function LastFetchAllCard() {
    const stamp = useStore(viewsCountStoreV3, (s) => s.lastFetchAllAt)
    const now = useNow()
    ...
    if (stamp === null) {
        return (
            <Tooltip
                trigger={
                    <StatCardShell label="Last fetch all">
                        <Heading size="lg">never</Heading>
                    </StatCardShell>
                }
            >
                <TooltipContent title="Will fire on the next leader takeover" />
            </Tooltip>
        )
    }
    ...
    return (
        <Tooltip
            trigger={
                <StatCardShell label="Last fetch all">
                    <Heading size="lg">{formatAge(stamp, now)} ago</Heading>
                </StatCardShell>
            }
        >
            <TooltipContent title={new Date(stamp).toISOString()} />
        </Tooltip>
    )
}

function FetchAllCooldownCard({ config }: { config: RefreshConfigV3 }) {
    const stamp = useStore(viewsCountStoreV3, (s) => s.lastFetchAllAt)
    const now = useNow()

    const cooldownSeconds = config.fetchAllMinCooldownSeconds

    if (stamp === null) {
        return (
            <Tooltip
                trigger={
                    <StatCardShell label="Fetch all cooldown">
                        <Heading size="lg">ready</Heading>
                    </StatCardShell>
                }
            >
                <TooltipContent
                    title={`No fetch-all has fired yet. Next leader takeover will fire one (cooldown: ${cooldownSeconds}s).`}
                />
            </Tooltip>
        )
    }
    ...
    const remainingMs = cooldownMs - age
    ...
    return (
        ...
        >
            <TooltipContent
                title={
                    inCooldown
                        ? `Configured: ${cooldownSeconds}s. Next leader takeover after this elapses will re-fire.`
                        : `Configured: ${cooldownSeconds}s. Next leader takeover will re-fire.`
                }
            />
        </Tooltip>
    )
...
function RecentViewsCard({ config }: { config: RefreshConfigV3 }) {
    const recent = useStore(viewsCountStoreV3, (s) => s.recent)
...
function StatsBar({
    ...
    return (
        <Box style={statsRowStyle} mb="md">
            ...
            <FetchAllCooldownCard config={config} />
        </Box>
    )
}
...
function buildRecentColumns(config: RefreshConfigV3) {
...
function getRecentColumns(config: RefreshConfigV3) {
