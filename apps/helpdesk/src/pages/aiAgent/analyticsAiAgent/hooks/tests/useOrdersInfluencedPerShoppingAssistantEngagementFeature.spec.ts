...
} from '../useOrdersInfluencedPerShoppingAssistantEngagementFeature'
...
jest.mock(
    ...
    () => ({
        aiAgentSalesOrdersInfluencedPerEngagementTypeQueryV2Factory: jest.fn(),
    }),
)
...
const mockQueryFactory = jest.requireMock(
    'domains/reporting/models/scopes/aiSalesAgentOrdersPerformance',
).aiAgentSalesOrdersInfluencedPerEngagementTypeQueryV2Factory as jest.Mock
...
const defaultAllValues = [
    { dimension: 'search_bar', value: 48, decile: null },
    { dimension: 'ask_anything', value: 14, decile: null },
]

describe('useOrdersInfluencedPerShoppingAssistantEngagementFeature', () => {
    beforeEach(() => {
        jest.clearAllMocks()
...
describe('fetchOrdersInfluencedPerShoppingAssistantEngagementFeature', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchOrdersInfluencedPerShoppingAssistantEngagementFeature(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
