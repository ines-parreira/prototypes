...
import type {
    ChannelsVoiceAgentEntityMetrics,
    ChannelsVoiceAgentMetricKey,
    ChannelsVoiceAgentMetricsData,
} from 'domains/reporting/pages/performance/channels/voice/charts/breakdownTables/ChannelsVoiceAgentTable/columns'
import {
    buildChannelsVoiceAgentEntityRow,
    CHANNELS_VOICE_AGENT_COLUMNS,
    CHANNELS_VOICE_AGENT_METRIC_FACTORIES,
...
const CHANNELS_VOICE_AGENT_METRICS_CONFIG = Object.fromEntries(
    Object.entries(CHANNELS_VOICE_AGENT_METRIC_FACTORIES).map(
        ([key, factory]) => [
            key,
            {
...
) as Record<ChannelsVoiceAgentMetricKey, EntityMetricConfig>
...
export const useChannelsVoiceAgentMetrics =
    (): ChannelsVoiceAgentMetricsData => {
        ...
        const {
            ...
            CHANNELS_VOICE_AGENT_METRICS_CONFIG,
            cleanStatsFilters,
            userTimezone,
        )
        ...
        const data = useMemo(() => {
            ...
            return assembleEntityRows(
                ids,
                buildChannelsVoiceAgentEntityRow(entityData),
            )
        }, [entityData, agents])
...
const CHANNELS_VOICE_AGENT_FILENAME = 'performance-channels-voice_by-agent'

export const fetchChannelsVoiceAgentMetrics = async (
    ...
    const rows = assembleEntityRows(
        ids,
        buildChannelsVoiceAgentEntityRow(metrics.data),
    )
    ...
    const headers = [
        CHANNELS_VOICE_AGENT_TABLE.title,
        ...CHANNELS_VOICE_AGENT_COLUMNS.map((col) => col.label),
    ]
    const csvRows = rows.map((row) => [
        humanizeAgent(agents, row.entity),
        ...CHANNELS_VOICE_AGENT_COLUMNS.map((col) =>
            formatMetricValue(
                row[
                    col.accessorKey as keyof ChannelsVoiceAgentEntityMetrics
                ] as number,
                col.metricFormat,
            ),
        ),
...
export const fetchChannelsVoiceAgentAsConfigurableTable: ConfigurableGraphFetch =
    async (_savedMeasure, _savedDimension, filters, timezone) => {
        const agents = getFilteredAgents(store.getState())
        const { files } = await fetchChannelsVoiceAgentMetrics(
            filters,
            timezone,
            agents,
        )
