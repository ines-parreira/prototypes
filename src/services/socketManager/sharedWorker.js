...
const MAX_INCREMENTAL_RECONNECT_BACKOFF = 30
const DISCONNECTED_NOTIFICATION_DELAY = 10
...
class WebsocketSharedWorker {
    ...
    constructor() {
        setInterval(this.sendHealthCheck, 1000)
    }
    ...
    sendHealthCheck = () => {
        ...
        setTimeout(this.disconnectTabs, 500)
    }
    ...
    onClientConnected = (message, messagePort) => {
        if (!this.socket) {
            this.ws_url = message.wsUrl
            this.socket = io(this.ws_url, {transports: ['websocket']}) // eslint-disable-line
