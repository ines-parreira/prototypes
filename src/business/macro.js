...
export function clearMacroBeforeApply(
    ...
    if (isInvalid) {
        return {
            ...
            notification: {
                message: 'We have removed the attachment from this message, because you cannot send text and ' +
    ...
    if (isInvalid) {
        return {
            macro: removeAttachmentsFromActions(macro),
            notification: {
                message: 'We have removed the attachments from this message, because you cannot send multiple ' +
...
function getAttachmentsCount(macro: RecordOf<Macro>): number {
    return macro.get('actions', fromJS([]))
        .filter((action: RecordOf<MacroAction>) => action.get('name') === 'addAttachments')
        .reduce(
...
function hasText(macro: RecordOf<Macro>): boolean {
    return !macro.get('actions', fromJS([]))
        .filter((action: RecordOf<MacroAction>) => action.get('name') === 'setResponseText')
        .isEmpty()
}
