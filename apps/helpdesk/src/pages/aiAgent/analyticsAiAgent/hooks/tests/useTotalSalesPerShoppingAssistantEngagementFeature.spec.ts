...
} from '../useTotalSalesPerShoppingAssistantEngagementFeature'
...
jest.mock(
    ...
    () => ({
        aiAgentSalesTotalSalesPerEngagementTypeQueryV2Factory: jest.fn(),
    }),
)
...
const mockQueryFactory = jest.requireMock(
    'domains/reporting/models/scopes/aiSalesAgentOrdersPerformance',
).aiAgentSalesTotalSalesPerEngagementTypeQueryV2Factory as jest.Mock
...
const MOCK_QUERY = {
    metricName: 'ai-agent-sales-performance-total-sales-per-engagement-type',
}
const defaultAllValues = [
    { dimension: 'search_bar', value: 3500, decile: null },
    { dimension: 'ask_anything', value: 1200, decile: null },
]

describe('useTotalSalesPerShoppingAssistantEngagementFeature', () => {
    beforeEach(() => {
        jest.clearAllMocks()
...
describe('fetchTotalSalesPerShoppingAssistantEngagementFeature', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchTotalSalesPerShoppingAssistantEngagementFeature(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
