...
describe('<TicketSubmitButtons />', () => {
    const mockSetTicketStatus = jest.fn()
    ...
    describe('handleSendAndCloseTicket', () => {
        ...
        describe('when hasUIVisionMS1 flag is disabled (legacy behavior)', () => {
            ...
            it('should call setTicketStatus with Closed status when clicking Send & Close', async () => {
            ...
            it('should not call submit or closeTicket in legacy mode', async () => {
                const { user, getByRole } = renderComponent(
                    mockStore(existingTicketState),
                )
        ...
        describe('when hasUIVisionMS1 flag is enabled', () => {
            ...
            it('should return early if ticket has no id', async () => {
                const { user, getByRole } = renderComponent(mockStore(state))
                ...
                expect(mockValidateTicketFields).not.toHaveBeenCalled()
                expect(mockSubmit).not.toHaveBeenCalled()
                expect(mockCloseTicket).not.toHaveBeenCalled()
            ...
            it('should not call setTicketStatus in new behavior mode', async () => {
                mockValidateTicketFields.mockReturnValue({
                    hasErrors: false,
                    invalidFieldIds: [],
                })
