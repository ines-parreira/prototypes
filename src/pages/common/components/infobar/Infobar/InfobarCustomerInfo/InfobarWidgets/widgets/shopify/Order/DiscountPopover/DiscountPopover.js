...
import {formatPrice, getDiscountAmount} from '../../../../../../../../../../../business/shopify/order'
...
export default class DiscountPopover extends React.PureComponent<Props, State> {
    ...
    componentDidUpdate(prevProps: Props, prevState: State) {
        ...
        if (onOpen) {
            ...
            segmentTracker.logEvent(segmentTracker.EVENTS.SHOPIFY_DUPLICATE_ORDER_DISCOUNT_POPOVER_OPEN)
        } else if (onClose) {
    ...
    _onDiscountValueChange = (event: SyntheticInputEvent<HTMLInputElement>) => {
    ...
    _onSubmit = (event: SyntheticEvent<HTMLFormElement>) => {
        ...
        const value = formatPrice(discountValue)
        ...
        const newValue: Shopify.AppliedDiscount = {
            ...
            value,
            value_type: type,
            amount: formatPrice(getDiscountAmount(max, type, value)),
        }

        onChange(fromJS(newValue))

        segmentTracker.logEvent(segmentTracker.EVENTS.SHOPIFY_DUPLICATE_ORDER_DISCOUNT_POPOVER_APPLY)
    }
    ...
    _onRemove = () => {
        ...
        segmentTracker.logEvent(segmentTracker.EVENTS.SHOPIFY_DUPLICATE_ORDER_DISCOUNT_POPOVER_REMOVE)
    }

    _onClose = () => {
        this._toggle()
        segmentTracker.logEvent(segmentTracker.EVENTS.SHOPIFY_DUPLICATE_ORDER_DISCOUNT_POPOVER_CLOSE)
    }

    render() {
        const {id, children, placement, currencyCode, editable, value, max} = this.props
                ...
                >
                    ...
                    >
                        <PopoverBody className="pt-3">
                            <FormGroup>
                                <Label for="discount-value">Discount this order by</Label>
                                <InputGroup>
