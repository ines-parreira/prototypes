...
} from '../useHandoverInteractionsPerSalesAgentChannel'
...
jest.mock('domains/reporting/models/scopes/handoverInteractions', () => ({
    aiSalesAgentHandoverInteractionsPerChannelQueryFactoryV2: jest.fn(),
}))
...
).aiSalesAgentHandoverInteractionsPerChannelQueryFactoryV2 as jest.Mock
...
const MOCK_QUERY = {
    metricName: 'ai-agent-shopping-assistant-handover-interactions-per-channel',
}
...
describe('useHandoverInteractionsPerSalesAgentChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
...
describe('fetchHandoverInteractionsPerSalesAgentChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchHandoverInteractionsPerSalesAgentChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
    ...
    it('returns the result from fetchStatsMetricPerDimension', async () => {
        const result = await fetchHandoverInteractionsPerSalesAgentChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
