...
} from '../useAutomatedInteractionsPerSupportAgentIntent'
...
jest.mock(
    ...
    () => ({
        aiSupportAgentAutomatedInteractionsPerIntentQueryFactoryV2: jest.fn(),
    }),
)
...
const mockQueryFactory = jest.requireMock(
    'domains/reporting/models/scopes/aiAgentAutomatedInteractions',
).aiSupportAgentAutomatedInteractionsPerIntentQueryFactoryV2 as jest.Mock
...
const defaultAllValues = [
    { dimension: 'Billing :: Refund', value: 1200, decile: null },
    { dimension: 'Shipping :: Status', value: 800, decile: null },
]

describe('useAutomatedInteractionsPerSupportAgentIntent', () => {
    beforeEach(() => {
        jest.clearAllMocks()
...
describe('fetchAutomatedInteractionsPerSupportAgentIntent', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchAutomatedInteractionsPerSupportAgentIntent(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
    ...
    it('returns the result from fetchStatsMetricPerDimension', async () => {
        const result = await fetchAutomatedInteractionsPerSupportAgentIntent(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
