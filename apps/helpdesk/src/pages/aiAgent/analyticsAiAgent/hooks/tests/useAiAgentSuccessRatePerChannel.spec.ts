...
} from '../useAiAgentSuccessRatePerChannel'
...
jest.mock('domains/reporting/models/scopes/aiAgentSuccessRate', () => ({
    aiAgentSuccessRatePerChannelQueryFactoryV2: jest.fn(),
}))
...
).aiAgentSuccessRatePerChannelQueryFactoryV2 as jest.Mock
...
const MOCK_QUERY = { metricName: 'ai-agent-success-rate-per-channel' }

const defaultAllValues = [
    { dimension: 'email', value: 0.78, decile: null },
    { dimension: 'chat', value: 0.91, decile: null },
]

describe('useAiAgentSuccessRatePerChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
    ...
    it('calls the query factory with filters and timezone', () => {
        renderHook(() =>
            useAiAgentSuccessRatePerChannel(MOCK_STATS_FILTERS, MOCK_TIMEZONE),
        )
...
describe('fetchAiAgentSuccessRatePerChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchAiAgentSuccessRatePerChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
    ...
    it('returns the result from fetchStatsMetricPerDimension', async () => {
        const result = await fetchAiAgentSuccessRatePerChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
