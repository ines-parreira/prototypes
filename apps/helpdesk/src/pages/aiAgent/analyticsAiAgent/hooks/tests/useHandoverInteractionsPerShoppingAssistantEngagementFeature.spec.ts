...
} from '../useHandoverInteractionsPerShoppingAssistantEngagementFeature'
...
jest.mock('domains/reporting/models/scopes/handoverInteractions', () => ({
    aiSalesAgentHandoverInteractionsPerEngagementTypeQueryFactoryV2: jest.fn(),
}))
...
).aiSalesAgentHandoverInteractionsPerEngagementTypeQueryFactoryV2 as jest.Mock
...
const MOCK_QUERY = {
    metricName:
        'ai-agent-shopping-assistant-handover-interactions-per-engagement-type',
}
const defaultAllValues = [
    { dimension: 'search_bar', value: 120, decile: null },
    { dimension: 'ask_anything', value: 80, decile: null },
]

describe('useHandoverInteractionsPerShoppingAssistantEngagementFeature', () => {
    beforeEach(() => {
        jest.clearAllMocks()
...
describe('fetchHandoverInteractionsPerShoppingAssistantEngagementFeature', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchHandoverInteractionsPerShoppingAssistantEngagementFeature(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
