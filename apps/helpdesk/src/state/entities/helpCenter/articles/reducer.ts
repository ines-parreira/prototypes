...
const DefaultExportReducer = createReducer<HelpCenterArticlesState>(
    initialState,
    (builder) =>
        builder
            ...
            .addCase(
                articleActions.updateArticlesOrder,
                (state, { payload }) => {
                    payload.forEach((articleId, position) => {
                        if (state.articlesById[articleId.toString()]) {
                            state.articlesById[articleId.toString()].position =
                                position
                        }
                    })
                },
            ...
            .addCase(
                articleActions.removeLocaleFromArticle,
                (state, { payload }) => {
