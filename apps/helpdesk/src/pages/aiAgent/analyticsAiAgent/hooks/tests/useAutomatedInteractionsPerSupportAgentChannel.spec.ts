...
} from '../useAutomatedInteractionsPerSupportAgentChannel'
...
jest.mock(
    ...
    () => ({
        aiSupportAgentAutomatedInteractionsPerChannelQueryFactoryV2: jest.fn(),
    }),
)
...
const mockQueryFactory = jest.requireMock(
    'domains/reporting/models/scopes/aiAgentAutomatedInteractions',
).aiSupportAgentAutomatedInteractionsPerChannelQueryFactoryV2 as jest.Mock
...
describe('useAutomatedInteractionsPerSupportAgentChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
...
describe('fetchAutomatedInteractionsPerSupportAgentChannel', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchAutomatedInteractionsPerSupportAgentChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
    ...
    it('returns the result from fetchStatsMetricPerDimension', async () => {
        const result = await fetchAutomatedInteractionsPerSupportAgentChannel(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
