...
describe('getDropoffColor', () => {
    const tiers: WorkflowDropoffMetricTiers[] = [
        {range: [0, 17], color: LOW_TIERS_DROPOFF_COLOR},
        {range: [18, 35], color: MID_TIERS_DROPOFF_COLOR},
    ...
    it('should return the correct color for the edge values within the tiers', () => {
        expect(getDropoffColor(0, tiers)).toBe(LOW_TIERS_DROPOFF_COLOR)
        expect(getDropoffColor(17, tiers)).toBe(LOW_TIERS_DROPOFF_COLOR)
        expect(getDropoffColor(18, tiers)).toBe(MID_TIERS_DROPOFF_COLOR)
        expect(getDropoffColor(35, tiers)).toBe(MID_TIERS_DROPOFF_COLOR)
        expect(getDropoffColor(36, tiers)).toBe(HIGH_TIERS_DROPOFF_COLOR)
        expect(getDropoffColor(53, tiers)).toBe(HIGH_TIERS_DROPOFF_COLOR)
    })
})
