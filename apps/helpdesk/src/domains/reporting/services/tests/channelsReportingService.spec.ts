...
describe('channelsReportingService', () => {
    ...
    it('should format null metric values as N/A placeholder', () => {
        const createCsvSpy = jest
            .spyOn(files, 'createCsv')
            .mockReturnValue('fakeReport1')

        const nullMetric: MetricWithDecile = {
            isFetching: false,
            isError: false,
            data: null,
        }
        const exampleData = (
            ...
            data: {
                ...
                decile: 4,
                allData: [
                    { [CHANNEL_DIMENSION]: channel, [metricField]: value },
                ],
                dimensions: [CHANNEL_DIMENSION],
                measures: [metricField] as any,
            },
        ...
        const data = {
            ...
            humanTimeAfterAiHandoffMetricPerChannel: nullMetric,
            percentageOfCreatedTicketsMetricPerChannel: nullMetric,
            closedTicketsMetricPerChannel: nullMetric,
            ticketAverageHandleTimePerChannel: nullMetric,
            medianFirstResponseTimeMetricPerChannel: nullMetric,
            medianResponseTimeMetricPerChannel: nullMetric,
            medianResolutionTimeMetricPerChannel: nullMetric,
            ticketsRepliedMetricPerChannel: nullMetric,
            messagesSentMetricPerChannel: nullMetric,
            messagesReceivedMetricPerChannel: nullMetric,
            customerSatisfactionMetricPerChannel: nullMetric,
        }

        saveReport([channelA], data, columnsOrder, fileName)

        const csvRows = createCsvSpy.mock.calls[0][0] as string[][]
        const dataRow = csvRows[1]

        expect(dataRow[0]).toBe(channelA.slug)
        // createdTicketsMetricPerChannel is non-null (needed for nonEmptyChannels filter),
        // all other metrics are null → formatted as NOT_AVAILABLE_PLACEHOLDER
        expect(dataRow[1]).toBe('5')
        expect(dataRow.slice(2).every((v) => v === '-')).toBe(true)
    })
...
describe('getChannelMetric', () => {
    ...
    it('should use allValues when available and return matching value', () => {
        const result = getChannelMetric(channelSlug, {
            data: {
                ...
                allData: [],
                allValues: [
                    { dimension: 'other-channel', value: 10, decile: null },
                    { dimension: channelSlug, value: 42.5, decile: null },
                ],
                dimensions: [CHANNEL_DIMENSION],
                measures: [metricField] as any,
            },
    ...
    it('should fall back to allData when allValues has no match', () => {
        const result = getChannelMetric(channelSlug, {
            data: {
                value: null,
                decile: null,
