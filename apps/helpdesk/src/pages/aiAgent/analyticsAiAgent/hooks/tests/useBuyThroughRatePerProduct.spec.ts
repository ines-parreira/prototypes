...
} from '../useBuyThroughRatePerProduct'
...
const mockQueryFactory = jest.requireMock(
    'domains/reporting/models/scopes/aiSalesAgentBuyThroughRate',
).aiSalesAgentBuyThroughRatePerProductQueryFactoryV2 as jest.Mock
...
const MOCK_QUERY = { metricName: 'ai-agent-buy-through-rate-per-product' }
const defaultAllValues = [
    { dimension: '123', value: 0.1, decile: null },
    { dimension: '456', value: 0.15, decile: null },
]

describe('useBuyThroughRatePerProduct', () => {
    beforeEach(() => {
        jest.clearAllMocks()
...
describe('fetchBuyThroughRatePerProduct', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('returns the result from fetchStatsMetricPerDimension', async () => {
        const result = await fetchBuyThroughRatePerProduct(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
