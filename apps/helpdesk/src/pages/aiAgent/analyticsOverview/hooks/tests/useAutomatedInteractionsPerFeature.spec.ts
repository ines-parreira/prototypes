...
} from '../useAutomatedInteractionsPerFeature'
...
const defaultAllValues = [
    ...
    { dimension: 'article-recommendation', value: 300, decile: null },
    { dimension: 'order-management', value: 150, decile: null },
]

describe('useAutomatedInteractionsPerFeature', () => {
    beforeEach(() => {
        jest.clearAllMocks()
...
describe('fetchAutomatedInteractionsPerFeature', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchAutomatedInteractionsPerFeature(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
    ...
    it('returns the result from fetchStatsMetricPerDimension', async () => {
        const result = await fetchAutomatedInteractionsPerFeature(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
