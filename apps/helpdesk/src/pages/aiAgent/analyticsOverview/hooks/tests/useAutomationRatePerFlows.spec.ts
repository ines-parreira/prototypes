...
} from '../useAutomationRatePerFlows'
...
jest.mock('domains/reporting/models/scopes/overallAutomationRate', () => ({
    overallAutomationRatePerFlowsQueryFactoryV2: jest.fn(),
}))
...
).overallAutomationRatePerFlowsQueryFactoryV2 as jest.Mock
...
const MOCK_QUERY = { metricName: 'ai-agent-overall-automation-rate-per-flows' }

const defaultAllValues = [
    { dimension: 'flow-seed-10', value: 0.75, decile: null },
    { dimension: 'flow-seed-25', value: 0.9, decile: null },
    { dimension: 'flow-seed-6', value: 0.6, decile: null },
    { dimension: 'flow-seed-9', value: 0.8, decile: null },
]

describe('useAutomationRatePerFlows', () => {
    beforeEach(() => {
        jest.clearAllMocks()
    ...
    it('calls the query factory with filters and timezone', () => {
        renderHook(() =>
            useAutomationRatePerFlows(MOCK_STATS_FILTERS, MOCK_TIMEZONE),
        )
...
describe('fetchAutomationRatePerFlows', () => {
    beforeEach(() => {
        jest.clearAllMocks()
        mockQueryFactory.mockReturnValue(MOCK_QUERY)
    ...
    it('calls the query factory with filters and timezone', async () => {
        await fetchAutomationRatePerFlows(MOCK_STATS_FILTERS, MOCK_TIMEZONE)
    ...
    it('returns the result from fetchStatsMetricPerDimension', async () => {
        const result = await fetchAutomationRatePerFlows(
            MOCK_STATS_FILTERS,
            MOCK_TIMEZONE,
        )
